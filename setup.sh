...
phase_env() {
  ...
  "EMAIL_API_BASE_URL|Email API base URL|Base URL for the Email API used to send approved campaign emails and receive reply events.|Open https://www.emailapi.com/ in your browser\nSign in to your Email API account and open developer settings\nFind the API base URL for your workspace or environment\nCopy the API base URL and paste it into the field above."
  "EMAIL_API_KEY|Email API key|API key for sending campaign emails and reading prospect replies through the Email API.|Open https://www.emailapi.com/ in your browser\nSign in to your Email API account and open developer settings\nCreate or open an API key with permission to send email and read reply events\nCopy the API key and paste it into the field above."
  "SLACK_BOT_TOKEN|Slack bot token|Slack bot token used for campaign setup inputs, approval prompts, status updates, coordination, and reply review notifications.|Open https://api.slack.com/apps in your browser\nSelect the Slack app for Sarah or create one for your workspace\nClick OAuth & Permissions in the left sidebar\nClick Install to Workspace if the app is not installed yet\nCopy the Bot User OAuth Token and paste it into the field above."
  )
...
phase_db() {
  ...
  if [[ -n "${PG_CONNECTION_STRING:-}" ]]; then
    ...
    if ${local_pg}; then
      ...
      warn "     Then set in .env: PG_CONNECTION_STRING=postgresql://postgres:<choose-a-password>@localhost:5432/sarah"
      exit 1
    fi
...
phase_finish() {
  ...
  if command -v openclaw >/dev/null 2>&1; then
    if openclaw cron list 2>/dev/null | grep -q "^campaign-sequence-send\b"; then
      ok "cron 'campaign-sequence-send' already registered"
    else
      openclaw cron add --file cron/campaign-sequence-send.json && ok "registered cron 'campaign-sequence-send'"
    fi
  else
    warn "openclaw CLI not found — register manually: openclaw cron add --file cron/campaign-sequence-send.json"
  fi
  if command -v openclaw >/dev/null 2>&1; then
    if openclaw cron list 2>/dev/null | grep -q "^campaign-follow-up-send\b"; then
      ok "cron 'campaign-follow-up-send' already registered"
    else
      openclaw cron add --file cron/campaign-follow-up-send.json && ok "registered cron 'campaign-follow-up-send'"
    fi
  else
    warn "openclaw CLI not found — register manually: openclaw cron add --file cron/campaign-follow-up-send.json"
  fi
  if command -v openclaw >/dev/null 2>&1; then
    if openclaw cron list 2>/dev/null | grep -q "^email-reply-received\b"; then
      ok "cron 'email-reply-received' already registered"
    else
      openclaw cron add --file cron/email-reply-received.json && ok "registered cron 'email-reply-received'"
    fi
  else
    warn "openclaw CLI not found — register manually: openclaw cron add --file cron/email-reply-received.json"
  fi
  ...
  log "  openclaw cron run --name campaign-sequence-send    # trigger manually"
  log "  openclaw cron run --name campaign-follow-up-send    # trigger manually"
  log "  openclaw cron run --name email-reply-received    # trigger manually"
